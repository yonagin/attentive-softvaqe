    ...
    with torch.no_grad():
        for batch_idx, (x, labels) in enumerate(data_loader):
            ...
            if hasattr(model, 'vector_quantization'):
                ...
                # Calculate distances to codebook to get indices
                z_flat = z.permute(0, 2, 3, 1).contiguous()
                z_flat = z_flat.view(-1, model.quantizer.embedding_dim)
                
                distances = (z_flat.unsqueeze(1) - model.quantizer.codebook.weight.unsqueeze(0)).pow(2).sum(2)
